...
abstract class AbstractThreadPool extends AbstractExecutorService implements ThreadPool {
    ...
    private final AtomicReference<ThreadPoolState> state = new AtomicReference<>(ThreadPoolState.STARTED);
    ...
    AbstractThreadPool(long taskTimeoutNanos, long watchdogIntervalNanos) {
        ...
        watchdog = taskTimeoutNanos != 0 ? new Watchdog(taskTimeoutNanos, watchdogIntervalNanos) : null;
    }
    ...
    final boolean doShutdown(boolean interrupt, @Nullable List<Runnable> unprocessedTasks) {
        ...
        if (interrupt) {
            if (enterState(ThreadPoolState.STARTED, ThreadPoolState.SHUT_DOWN_WITH_INTERRUPTS)) {
                shutdownAlreadyInProgress = false;
            } else {
                // needsShutdownTasks stays false because the shutdown tasks were added already when shutdownState exits NOT_SHUTDOWN.
                enterState(ThreadPoolState.SHUT_DOWN_WITHOUT_INTERRUPTS, ThreadPoolState.SHUT_DOWN_WITH_INTERRUPTS);
                logger.debug("shutdownNow() was called while shutdown() is in progress.");
            }
        } else {
            if (enterState(ThreadPoolState.STARTED, ThreadPoolState.SHUT_DOWN_WITHOUT_INTERRUPTS)) {
                shutdownAlreadyInProgress = false;
            }
        }
